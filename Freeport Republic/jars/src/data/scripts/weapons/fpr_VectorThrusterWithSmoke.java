...
public class fpr_VectorThrusterWithSmoke implements EveryFrameWeaponEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if(time>=FREQ){
            ...
            if(thrust>0){
                ...
                if(!turn){
                    //thrust only, easy.
                    thrust(engine, weapon, accelerateAngle, thrust*(SHIP.getMutableStats().getAcceleration().computeMultMod()), SMOOTH_THRUSTING);
                } else {
                    if(!accel){                        
                        //turn only, easy too.
                        thrust(engine, weapon, turnAngle, thrust*(SHIP.getMutableStats().getTurnAcceleration().computeMultMod()), SMOOTH_THRUSTING);
...
//                        );
                        ...
                        if(FRAMES==0){
                            ...
                            thrust(engine, weapon, combinedAngle, combinedThrust, SMOOTH_THRUSTING/2);
                        }
...
//                );
            //DEBUG
                ...
                if(FRAMES==0){          
                    ...
                    thrust(engine, weapon, NEUTRAL_ANGLE, 0, SMOOTH_THRUSTING);
                }
            }
        }
    }
    ...
    private void thrust(CombatEngineAPI engine, WeaponAPI weapon, float angle, float thrust, float smooth){
        ...
        //engine.addFloatingText(weapon.getLocation(), length + "", 20, Color.WHITE, weapon.getShip(), 0, 0.5f);
        ...
        if (thrust > 0.2f){
            engine.addSmoothParticle(
                    weapon.getLocation(),
                    MathUtils.getPoint(new Vector2f(), 400*thrust, weapon.getCurrAngle()),
                    50*thrust,
                    1f,
                    (thrust+(float)Math.random()*0.5f)/3,
                    new Color(0.02f, 0.066f, 0.1f)
            );
        }
    }
